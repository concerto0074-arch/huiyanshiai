...
from deepseek_report import polish_report_with_deepseek

def test_deepseek_report_generation():
    """
    测试使用DeepSeek润色报告的功能
    """
    ...
    # 生成报告的函数（不包含DeepSeek润色，用于对比）
    def generate_original_report(prob, risk_level, patient_features):
        # 构建患者特征字典
    ...
    for i, row in enumerate(features):
        ...
        original_report = generate_original_report(prob, risk_level, patient_features)
        
        # 使用DeepSeek润色报告
        polished_report = polish_report_with_deepseek(original_report)
...
if __name__ == "__main__":
    try:
        print("开始测试DeepSeek报告润色功能...")
        results = test_deepseek_report_generation()
        print("测试完成！")
