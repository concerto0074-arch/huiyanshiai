...
# setwd('.')
#?ϲ?????
ADC<-read.csv('1157gene-ADC.csv')
NL<-read.csv('1157gene-NL.csv')
SCC<-read.csv('1157gene-SCC.csv')
COID<-read.csv('1157gene-COID.csv')
GI1<-ADC[,c(1,2)]
GI2<-NL[,c(1,2)]
GI3<-SCC[,c(1,2)]
GI4<-COID[,c(1,2)]
...
ytest<-as.factor(ytest)## ??leibioaoqian????y,???Ը?Ϊ????
tezheng<-gdata[,2]

tezheng<-as.matrix(tezheng)##??tezhengqunweizhi????tezheng
###ind weight
...
for (i in 1:10){
  ...
  fit <- msgl::fit(xtrain,classes=ytrain,grouping=tezheng, 
                   ...
                   alpha = 0.8,
                   lambda = 0.05)
  n<-features(fit)[[best_model(fit.cv)]] # ????ģ???еķ???????
  nr<-c(nr,n)#xishu
  hr[i]<-length(n)
  ...
  fit <- msgl::fit(xtrain,classes=ytrain,grouping=tezheng, 
                   ...
                   alpha = 0.5,
                   lambda = 0.05)
  nn<-features(fit)[[best_model(fit.cv)]] # ????ģ???еķ???????
  nnr<-c(nnr,nn)#xishu
}
...
sd(hr)
write.csv(nr,'nr.csv')
write.csv(nnr,'nnr.csv')
write.csv(AUC,'AUC-proposed-noise.csv')
