...
# setwd('.')
#合并数据
ADC<-read.csv('cleanadc.csv')
NL<-read.csv('cleannl.csv')
SCC<-read.csv('cleanscc.csv')
COID<-read.csv('cleancoid.csv')
GI1<-ADC[,c(1,2)]
GI2<-NL[,c(1,2)]
GI3<-SCC[,c(1,2)]
GI4<-COID[,c(1,2)]
...
###ind weight
noiseADC<-read.csv('noiseadc.csv')
noiseNL<-read.csv('noisenl.csv')
noiseSCC<-read.csv('noisescc.csv')
noiseCOID<-read.csv('noisecoid.csv')
noiseADC1<-noiseADC[,-c(1,2)]
noiseNL1<-noiseNL[,-c(1,2)]
noiseSCC1<-noiseSCC[,-c(1,2)]
noiseCOID1<-noiseCOID[,-c(1,2)]
gdata2<-rbind(noiseADC1,noiseNL1,noiseSCC1,noiseCOID1)
indw<-rowSums(gdata2)
w1<-abs(1/indw)
summary(w1)
...
mean(hr)
sd(hr)
write.csv(AUC,'AUC-RAMRSGL-clean.csv')
