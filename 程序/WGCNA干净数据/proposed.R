...
# setwd('.')
#合并数据
ADC<-read.csv('cleandata-ADC.csv')
NL<-read.csv('cleandata-NL.csv')
SCC<-read.csv('cleandata-SCC.csv')
COID<-read.csv('cleandata-COID.csv')
GI1<-ADC[,c(1,2)]
GI2<-NL[,c(1,2)]
GI3<-SCC[,c(1,2)]
GI4<-COID[,c(1,2)]
...
#write.csv(gdata,'gdata.csv',row.names = F)
...
tezheng<-as.matrix(tezheng)##将tezhengqunweizhi赋给tezheng
###ind weight
w3<-read.csv('./互信息/quanzhong.csv')
MM<-read.csv('./模块隶属度/MM.csv')
indw<-w3[,4]*abs(MM)
w1<-1/indw
summary(w1)
indw1<-as.matrix(t(cbind(w1,w1,w1,w1)))
###群权重
grpw<-read.csv('paixusu.csv')
wgrp<-1/grpw[,2]
...
for (i in 1:10){
  ...
  fit.cv <- msgl::cv(xtrain, classes=ytrain,grouping=tezheng,
                     
                     parameterWeights = indw1,
                     groupWeights = wgrp,
  ...
  ################拟合最终模型###################################
  fit <- msgl::fit(xtrain,classes=ytrain,grouping=tezheng, 
                   parameterWeights = indw1,
                   groupWeights = wgrp,
  ...
  fit <- msgl::fit(xtrain,classes=ytrain,grouping=tezheng, 
                   ...
                   alpha = 0.5,
                   lambda = 0.05)
  nn<-features(fit)[[best_model(fit.cv)]] # 最优模型中的非零特征
  nnrclean<-c(nnrclean,nn)#xishu
}
...
mean(hr)
sd(hr)
write.csv(nrclean,'nrclean.csv')
write.csv(nnrclean,'nnrclean.csv')
write.csv(AUC,'AUC-proposed-clean.csv')
